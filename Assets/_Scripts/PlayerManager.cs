...
public class PlayerManager : MonoBehaviour
{
    ...
    private PlayerInvulnerability blackInvuln;
    private PlayerInvulnerability whiteInvuln;
    private PlayerInvulnerability grayInvuln;
    ...
    void Start()
    {
        ...
        blackInvuln = playerBlack.GetComponent<PlayerInvulnerability>();
        whiteInvuln = playerWhite.GetComponent<PlayerInvulnerability>();
        grayInvuln = playerGray.GetComponent<PlayerInvulnerability>();
    }
    ...
    private IEnumerator MoveAndActivateGray(GameObject mover, Vector3 targetPosition, bool blackActive)
    {
        ...
        playerGray.GetComponent<Animator>().runtimeAnimatorController = (blackActive ? blackGray : whiteGray);
        playerGray.SetActive(true);

        // Black/White 비활성화
        if (blackInvuln != null) blackInvuln.ResetInvuln();
        playerBlack.SetActive(false);

        if (whiteInvuln != null) whiteInvuln.ResetInvuln();
    ...
    {
        if (grayInvuln != null) grayInvuln.ResetInvuln();
